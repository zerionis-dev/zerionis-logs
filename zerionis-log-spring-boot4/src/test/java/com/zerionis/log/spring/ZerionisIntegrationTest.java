...
/**
 * Integration tests for zerionis-log Spring Boot 4 auto-configuration.
...
class ZerionisIntegrationTest {
    ...
    class TracePropagationTests {
        ...
        void interceptorAddsTraceIdHeader() {
            ...
            try {
                ...
                // Invoke interceptor — it should set headers
                try {
                    interceptor.intercept(request, new byte[0], (req, body) -> null);
                } catch (Exception ignored) {
                    // null execution returns null response, that's fine for this test
                }
    ...
    class AsyncContextTests {
        ...
        void decoratorPropagatesMdc() throws Exception {
            ...
            MDC.clear(); // Clear parent thread MDC to prove decorator captured it
        ...
        void decoratorCleansUp() throws Exception {
            ...
            Runnable decorated = decorator.decorate(() -> {
                // MDC is set during execution
            });

            // Wrap to check MDC after the decorated runnable completes
            Runnable wrapper = () -> {
                decorated.run();
